...
export const AssignElderlyToCaregiverModal = ({
  isOpen,
  onClose,
  caregiver = null,
  onAssign,
  ...
  const assignedIds = caregiver.assigned_elderly_ids || [];
  const assignedNames = caregiver.assigned_elderly_names || [];
  ...
  const handleAddAssignment = async (e) => {
    ...
    try {
      ...
      await onAssign(caregiver.id, selectedElderlyId);
    } catch (err) {
  ...
  const handleRemoveAssignment = async (elderlyId) => {
    try {
      setActionLoading(true);
      await onRemove(caregiver.id, elderlyId);
    } catch (err) {
    ...
    <div style={{
      ...
      <div className="glass-card animate-fade-in" style={{ width: '100%', maxWidth: '520px', padding: '28px', backgroundColor: '#ffffff' }}>
        ...
        <div style={{ marginBottom: '16px', fontSize: '0.92rem' }}>
          Caregiver Staff: <strong>{displayName}</strong> ({caregiver.shift} Shift)
        </div>
        ...
        <div>
          ...
          {assignedIds.length === 0 ? (
            <div style={{ color: 'var(--text-muted)', fontSize: '0.86rem', padding: '16px', textAlign: 'center', border: '1px dashed #cbd5e1', borderRadius: '8px' }}>
              No elderly patients currently assigned to this caregiver.
            </div>
          ) : (
...
export default AssignElderlyToCaregiverModal;
